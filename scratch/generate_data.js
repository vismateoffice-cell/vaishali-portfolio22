...
function getCleanCategoryName(folderName) {
  // Replace underscores, double spaces, and clean up common spelling
  let cleaned = folderName
    .replace(/_/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  
  // Custom mapping for cleaner display names if desired
  const customNames = {
...
folders.forEach(folder => {
  ...
  // category key will be a URL-safe lowercase identifier
  ...
  files.forEach(file => {
    ...
    items.push({
      ...
      // Since portfolio is in public/, reference from root /portfolio/...
...
// Read existing data.ts file to preserve experience, education and skills data
...
// Find where portfolioItems starts in the original file
// We'll replace from 'export const portfolioItems' to the end of the file or just replace that variable
